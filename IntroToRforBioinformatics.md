...
**Challenge 4:** Set a seed using `set.seed(215)` and use it to generate a random genome (`randGenome`), collapsed down to a single string. Once you've done this, count the frequency of Adenine (A) in the resulting "genome" by hand.
...
Did everyone's answer agree? We all had the same substring, but its unlikely that we all counted accurately. We've run into the issue of human error -- now imagine if this was a full genome, on the order of millions of nucleotides in length!
...
1. Set an iterator (this may just be a counter or it may be floating over elements in a list).
2. Provide the instructions to be run each time through the loop.

When the code is run, the instructions will be run once for each value of the iterator. Let's build a simple for loop that will just add up the values of the iterator. We'll set the iterator to run through the range of integer values from 0 through 9.
...
Consider the following loop which will only print out the occurrences of Adenine.
...
If you got that, you are ready to compute the frequencies of all of the nucleotides in the genome string.
...
+ Use [this link](http://bioinformaticsalgorithms.com/data/realdatasets/Replication/Vibrio_cholerae.txt) to access a full copy of the *Vibrio Cholerae*  chromosome, consisting of 1,108,250 nucleotides. 
...
+ Navigate to the [Counting DNA Nucleotides preblem](https://rosalind.info/problems/dna/)
...
+ Download the sample dataset, read it into R, use your code to count the number of each nucleotide, submit your solution.
...
Nice work. Although the culmination of this notebook is quite simple -- count the frequency of each nucleotide within a genome -- we've accomplished quite a bit and you should be proud!
