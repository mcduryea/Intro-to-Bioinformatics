...
Take a minute to review the touch command and how we used it to create multiple files. What is the significance of the curly brackets '{}'?
...
This is similar to using the "find" function on a browser. The wildcard (*) means that any number of other characters can follow. UNIX will look for all files, as long 'zmaysB' is included in the name.

Let's say we wanted to look for the first reads (R1 files) from samples 'A' and 'B'. We can do that using another UNIX wildcard, the square brackets:
```{bash}
ls zmays[AB]_R1.fastq
```

Notice that this command found the R1.fastq file for samples 'A' and 'B'.

Now, let's say we wanted to look for the R1 file for all three samples. We could do this by specifying a *range" with the square brackets and a hyphen:
```{bash}
ls zmays[A-C]_R1.fastq
```
